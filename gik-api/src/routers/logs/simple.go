...
func GetSimpleLogs(c *gin.Context) {
	...
	if userFilter != "" {
		...
		if err != nil {
			...
			user := type_news.User{}
			err = database.Database.Model(&type_news.User{}).Where("username = ?", userFilter).Find(&user).Error
			if err != nil {
				c.JSON(400, gin.H{
					"success": false,
	...
	baseQuery := database.Database.Model(&type_news.SimpleLog{})
