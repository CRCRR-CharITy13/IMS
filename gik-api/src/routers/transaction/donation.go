...
type ListDonationResponse struct {
	...
	TotalValue  float32 `json:"totalCost" binding:"required"`
}
...
func ListDonations(c *gin.Context) {
	...
	for idx, donation := range donations {
		...
		donationList[idx] = ListDonationResponse{
			...
			DonorName:   donation.DonorBy.Name,
...
type AddDonationResponse struct {
	ID           int    `json:"ID" binding:"required"`
...
func AddDonation(c *gin.Context) {
	...
	// lstAddDonationResponse := []AddDonationResponse{}
	...
	// 	if item.StockTotal < inputDonationItem.Quantity {
	// 		isSuccess = false
	// 		msgResponse = fmt.Sprintf("There are only %d of %s\n", item.StockTotal, inputDonationItem.SKUName)
	// 		break
	// 	}
	...
	// 	for _, donationItem := range lstDonationItem {
	// 		database.Database.First(&donationItem.Item, donationItem.ItemID)
	// 		// get list of item's warehouses

	// 		donationItem.Item.ValueTotal += donationItem.Count
	// 	}

	// 	//update Donor.Balance
	// 	database.Database.Save(donor)
	// }

	// c.JSON(200, gin.H{
	// 	"success": isSuccess,
	// 	"message": msgResponse,
	// 	"data":    lstAddDonationResponse,
	// })
